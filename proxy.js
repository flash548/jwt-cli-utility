...
function runUtil() {
    ...
    var jwtToken = parsedArgs.silent && tokenFound && parsedArgs.jwt != "none" ? parsedArgs.jwt : null;
  var namespace = parsedArgs.silent ? (parsedArgs.namespace == "none" ? null : parsedArgs.namespace || null) : "none";
    ...
    if (!parsedArgs.silent) {
        ...
        namespaceButtons.forEach((nsBtn) => {
            nsBtn.on("check", () => {
                namespace =
                    nsBtn.text.toLowerCase() == "none" ? null : nsBtn.text;
            });
        });
